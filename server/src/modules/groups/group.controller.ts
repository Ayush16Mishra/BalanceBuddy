...
import { createGroupSchema,updateGroupSchema,joinGroupSchema,getGroupBalancesSchema,settleGroupBalanceSchema } from "./group.validation.js";
...
async settleGroupBalance(req: Request, res: Response) {
  const { groupId, userId } = settleGroupBalanceSchema.parse(
    req.params
  );

  const result = await groupService.settleGroupBalance(
    req.user!.id,
    userId,
    groupId
  );
