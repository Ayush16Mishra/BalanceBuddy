...
import { emitExpenseSharePaid } from "../../socket/emitters.js";
...
export const expenseSharesService = {
  async markSharePaid(userId: string, shareId: string) {
    ...
    if (share.expense.status !== ExpenseStatus.ACTIVE) {
      throw new ApiError(400, "Cannot settle a cancelled expense.");
    }

    if (share.status === ExpenseShareStatus.PAID) {
      throw new ApiError(409, "Expense share is already paid.");
    }

    if (share.debtorId !== userId) {
      throw new ApiError(403, "You can only settle your own expense share.");
    }

    const updatedShare = await prisma.$transaction(async (tx) => {
      const updatedShare = await expenseSharesRepository.updateShare(tx, shareId, {
      ...
      await expenseSharesRepository.updateExpense(tx, share.expenseId, expenseData);

      return updatedShare;
    });
    await redis.del(`dashboard:${userId}`);
    // Emit only after the transaction has successfully committed
    emitExpenseSharePaid(share.expense.groupId, updatedShare);
    if (share.creditorId !== share.debtorId) {
      notificationService.notify(share.creditorId, {
        type: "settlement",
        title: "Payment Received",
        message: `${share.debtor.name} paid you for "${share.expense.title ?? "Untitled Expense"}".`,
      });
    }
    return serializeDecimal(updatedShare);
  },

  async getOutstandingBalances(userId: string, groupId?: string) {
    const shares = await expenseSharesRepository.getPendingShares(userId, groupId);
    ...
    for (const share of shares) {
      ...
      const otherUserId = isDebtor ? share.creditorId : share.debtorId;
  ...
  async settleGroupBalance(currentUserId: string, otherUserId: string, groupId: string) {
    const shares = await expenseSharesRepository.getPendingSharesBetweenUsers(
      groupId,
      currentUserId,
      otherUserId
    );
    ...
    if (shares.length === 0) {
      throw new ApiError(409, "No outstanding balances to settle.");
    }
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      for (const share of shares) {
        await expenseSharesRepository.updateShare(tx, share.id, {
          status: ExpenseShareStatus.PAID,
        });

        const newPaidCount = share.expense.paidCount + 1;

        await expenseSharesRepository.updateExpense(tx, share.expenseId, {
          paidCount: {
            increment: 1,
          },
          status:
        ...
        if (newPaidCount === share.expense.participantCount) {
          expensesSettled++;
        }
      }
    ...
    await Promise.all([
      redis.del(`dashboard:${currentUserId}`),
      redis.del(`dashboard:${otherUserId}`),
    ]);

    emitExpenseSharePaid(groupId, {
      groupId,
      settled: true,
    });
